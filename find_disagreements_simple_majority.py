...
def find_disagreements(file_path):
    ...
    # Find rows where there's no simple majority (less than two people agree)
    disagreements = df_scores[df_scores.apply(lambda row: row.value_counts().iloc[0] < 2, axis=1)]
...
disagreement_dir = 'disagreement_simple_majority' 
