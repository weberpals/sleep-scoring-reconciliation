...
def process_all_studies(data_path):
    for study in os.listdir(data_path):
        ...
        if os.path.isdir(study_path):
            ...
            with open(os.path.join(study_path, 'summary_3s.txt'), 'w') as f:
                f.write(summary)
            
            # Generate reconciliation file
            with open(os.path.join(study_path, 'reconciliation_needed_3s.txt'), 'w') as f:
                for bin_time in reconciliation_needed:
                    f.write(f"{bin_time.strftime('%H:%M:%S,%f')}\n")
